 ...
 */
class ManageCategoriesExecutor(
            ...
            scope.launch {
                try {
                    ...
                    updatedCategories.forEach { category ->
                        val domainCategory = categoryItemToDomain(category)
                        categoryRepository.updateCategory(domainCategory)
                    }
    ...
    private fun categoryItemToDomain(item: ManageCategoriesStore.CategoryItem): CategoryData {
        return CategoryData(
            id = item.id,
            name = item.name,
            iconName = item.iconName ?: "",
            color = item.color,
            type = Transaction.Type.valueOf(item.type),
    ...
    private fun saveCategory(category: ManageCategoriesStore.CategoryItem) {
        scope.launch {
            try {
                ...
                if (category.id == 0L) {
                    // Create new category
                    val newCategory = categoryRepository.createCategory(domainCategory)
