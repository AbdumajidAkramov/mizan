package dev.esbi.mizan.presentation.feature.addtransaction.domain.usecase
...
class AddTransactionUseCase @Inject constructor(
    ...
    private val voicePatterns = listOf(
        ...
        Pattern.compile(
            "(.+?)\\s+(?:\\$|dollars?|usd)?\\s*(\\d{1,10}(?:\\.\\d{2})?)",
            Pattern.CASE_INSENSITIVE
        ),

        // "$15000 Lunch" or "15000 Lunch"
        Pattern.compile(
            "(?:\\$|dollars?|usd)?\\s*(\\d{1,10}(?:\\.\\d{2})?)\\s+(.+)",
            Pattern.CASE_INSENSITIVE
        ),

        // "Lunch 15000 dollars"
        Pattern.compile(
            "(.+?)\\s+(\\d{1,10}(?:\\.\\d{2})?)\\s*(?:dollars?|usd)",
            Pattern.CASE_INSENSITIVE
        ),

        // "15000 for lunch"
        Pattern.compile(
            "(\\d{1,10}(?:\\.\\d{2})?)\\s+(?:for|to)\\s+(.+)",
            Pattern.CASE_INSENSITIVE
        ),
