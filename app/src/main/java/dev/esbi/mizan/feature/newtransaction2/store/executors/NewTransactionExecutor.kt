...
import dev.esbi.mizan.feature.newtransaction2.amountinput.voice.TransactionVoiceParser
import dev.esbi.mizan.feature.newtransaction2.amountinput.voice.VoiceRecognitionManager
                        ...
                        if (result.isSuccess) {
                            ...
                            if (currentState.saveAsTemplate) {
                                ...
                                val template = Template(
                                    name = categoryName,
                                    amount = BigDecimal.valueOf(currentState.amount),
...
//                dispatch(NewTransactionStore.Message.SetTypeSelectorVisible(true))
            ...
            is NewTransactionStore.Intent.SelectParentCategory -> {
                ...
                dispatch(NewTransactionStore.CategoryChooserMessage.ParentCategorySelected(category = intent.category))
                dispatch(NewTransactionStore.Message.SetCategorySheetVisible(false))
            }
...
//                publish(Label.NavigateToManageCategories)
    ...
    private fun selectParentCategory(category: Category) {
        if (state().categoryChooserState.selectedParentId == category.id) {
            dispatch(NewTransactionStore.CategoryChooserMessage.ParentCategorySelected(null))
            return
        }
        dispatch(NewTransactionStore.CategoryChooserMessage.ParentCategorySelected(category))
    }
