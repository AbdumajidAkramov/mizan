...
class AddTransactionUseCase @Inject constructor(
     ...
     * Examples:
     * "Lunch 15000" → TransactionData(amount = 15000.0, note = "Lunch", category = "Food")
     * "Uber 25000" → TransactionData(amount = 25000.0, note = "Uber", category = "Transport")
     * "Coffee 7500" → TransactionData(amount = 7500.0, note = "Coffee", category = "Food")
     * "15000 for lunch" → TransactionData(amount = 15000.0, note = "lunch", category = "Food")
     */
    suspend fun parseVoiceInput(text: String): TransactionData {
        val cleanText = text.trim()
        
        // Try each pattern to extract amount and note
        for (pattern in voicePatterns) {
            ...
            if (matcher.find()) {
                ...
                return when (groups) {
                    2 -> {
                        // Two groups: note and amount (or amount and note)
                        val group1 = matcher.group(1)?.trim() ?: ""
                        val group2 = matcher.group(2)?.trim() ?: ""
                        
                        // Determine which is amount and which is note
